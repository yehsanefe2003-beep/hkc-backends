...
export default function Makineler() {
  ...
  const { catalogItems } = useCatalogProducts('makineler')

  const items = useMemo(() => {
    let staticItems = products.filter(p => p.category === 'makineler')
    if (sub) staticItems = staticItems.filter(p => slug(p.subcategory) === sub)
    return [...staticItems, ...catalogItems]
  }, [sub, catalogItems])

  return <CategoryLayout title="Makineler" items={items} />
}
